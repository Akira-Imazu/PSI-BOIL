...
void Nucleation::replant () {
  ...
  for(int ns=0; ns<size(); ns++){
    ...
    sites[ns].set_sink_energy(0.0);        // initialize heat sink in this time step
	 ...
	 * (2) bubble does not exist and Tw < Tsat */
	   ...
	   * check if need to start plant or not */
          ...
          bool clr_seed_cond = false;
          if (clr_seed > threshold_clr_site) clr_seed_cond = true; /* true: in liquid */
	...
	/* micro-layer must not be change here: don't call insert_dmicro */
        ...
        if (size()==1) {
          boil::oout<<"replant:siteInfo "<<t_current<<" "<<tpr_seed<<" "
                    <<clr_seed<<" "<<clr_seed_cond<<" Pattern 2\n";
	}
  ...
  /* heat sink */
  //std::cout<<"replant:dsize= "<<dsize()<<"\n";
  for(int nsd=0; nsd<dsize(); nsd++){
    dsites[nsd].set_sink_energy(0.0);
    if (dsites[nsd].heat_sink_current()) {
      //std::cout<<"replant:heat_sink(dsites)\n";
  ...
  if (pre_heat_sink()) {
    ...
    for (int ns=0; ns<size(); ns++){
      ...
      tmp += sites[ns].sink_energy();      // sink_energy is current step
      sites[ns].set_sum_sink_energy(tmp);  // integrate over time step
    }
  }

  // sum of qsink over nucleation sites in this time step
  real sum_qsink_current_step=0.0;
  for(int ns=0; ns<size(); ns++){
    //std::cout<<"replant:proc= "<<boil::cart.iam()<<" "<<sites[ns].sink_energy()<<"\n";
    sum_qsink_current_step += sites[ns].sink_energy();
  }
  if (sum_qsink_current_step !=0.0 ){
    sum_qsink_current_step /= time->dt(); // change unit from J to W
  }
  boil::oout<<"replant:sum_qsink_current_step= "<<t_current<<" "
            <<sum_qsink_current_step<<" [W],dt= "<<time->dt()<<" [s]\n";
#if 0
  boil::oout<<"DEBUG:replant:sites[0].sink_energy()= "<<sites[0].sink_energy()<<"\n";
#endif
